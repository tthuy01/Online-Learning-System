...
public class UpdateSettingServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        String setId_raw = request.getParameter("id");
        int id = 0;
        try {
            id = Integer.parseInt(setId_raw);
        ...
        Setting s = setdao.getSettingById(id);
        request.setAttribute("id", s.getSettingId());
        request.setAttribute("type", s.getSettingTypeId());
        request.setAttribute("value", s.getSettingValue());
        request.setAttribute("order", s.getSettingOrder());
        request.setAttribute("status", s.getSettingStatusId());
        request.setAttribute("des", s.getSettingDescription());
        setdao.updateSetting(s);
        request.getRequestDispatcher("jsp/update_setting.jsp").forward(request, response);
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (type_raw.equals("0") || value == null || value.trim().equals("") || order == null || order.trim().equals("") || status_raw.equals("0") || des == null || des.trim().equals("")) {
            ...
            Setting s = new Setting(Integer.parseInt(id_raw), type, value, order, status, des);
            ...
            request.setAttribute("status", status_raw);
            request.setAttribute("des", des);
            request.setAttribute("ms", "Update setting successfully");
            request.getRequestDispatcher("jsp/update_setting.jsp").forward(request, response);
        }
    }
